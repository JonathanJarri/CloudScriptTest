...
handlers.DisableStoreObjects = function(args){
  var request = {"StoreId":"Store HL"}
  var headers = {
    "X-SecretKey": args.XSecretKey
  };

  var url = "https://14B7.playfabapi.com/Admin/GetStoreItems";
  ...
  for (var i = 0, len = result.data.Store.length; i < len; i++){
    result.data.Store[i].CustomData.isActive = false;
  }

  url = "https://14B7.playfabapi.com/Admin/SetStoreItems";
  content = JSON.stringify(result.data);
  httpMethod = "post";
  contentType = "application/json";

  // The pre-defined http object makes synchronous HTTP requests
  result = http.request(url, httpMethod, content, contentType, headers);
}

handlers.CheatAdminSaveUserData = function(args)
{
    let request = {
       PlayFabId : args.userId,
       Data : {
        "PLAYER_SAVE" : args.dataKey
        },
        "Permission" : "Public"
    };
    let response = server.UpdateUserData(request);
}
